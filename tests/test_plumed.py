...
def test_plumed_cv_from_atom_groups():
    ...
    with pytest.raises(TypeError):
        mlt.PlumedAverageCV('', 0)  # ty: ignore[invalid-argument-type]
...
def test_plumed_bias_from_cvs():
    ...
    with pytest.raises(ValueError):
        bias._set_metad_params(
            pace=10, width=(0.2, 0.3), height=0.5, biasfactor=0.5
        )
...
def test_plumed_plot():
    ...
    mlt.plot_cv1_and_cv2(
        filenames=(colvar1, colvar2),
        cvs_units=('Å', 'Å'),
