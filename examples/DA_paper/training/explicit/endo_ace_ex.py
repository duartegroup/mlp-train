...
def add_water(solute, n = 2):
    ...
    C98 = (sys[C_idx[7]].position-sys[C_idx[8]].position)/np.linalg.norm(sys[C_idx[7]].position-sys[C_idx[8]].position)
    C68 = (sys[C_idx[7]].position-sys[C_idx[5]].position)/np.linalg.norm(sys[C_idx[7]].position-sys[C_idx[5]].position)
    C48 = (sys[C_idx[7]].position-sys[C_idx[3]].position)/np.linalg.norm(sys[C_idx[7]].position-sys[C_idx[3]].position)
    C8O = (sys[O_idx[0]].position-sys[C_idx[7]].position)/np.linalg.norm(sys[O_idx[0]].position-sys[C_idx[7]].position)
...
def generate_init_configs(n, bulk_water = True, TS = True):
    ...
    TS.load_xyz(filename = 'cis_endo_TS_wB97M.xyz', charge = 0, TS.mult = 1)
    TS = TS[0]
    TS.box = Box([11, 11, 11])
    TS.charge = 0
    TS.mult = 1
