...
mlt.Config.orca_keywords = ['PBE','D3BJ', 'def2-TZVP',  'def2/J','RIJCOSX','EnGrad']
...
def generate_init_configs(n, solute_xyz, solvent_xyz, box_size = [18.5, 18.5, 18.5]):

    solute = mlt.ConfigurationSet()
    solute.load_xyz(filename = solute_xyz, charge = 1, mult = 1, box = mlt.box.Box(box_size))
    solute = solute[0]
    ...
    while len(init_configs)<n:
        ...
        solvated = solvation (solute_config=solute,
                              solvent_config=solvent,
                              apm=5,
                              radius=1.7)
...
if __name__ == "__main__":
    ...
    diff_r = mlt.PlumedDifferenceCV(name='diff_r', atom_groups=((4, 41), (4, 34)))
    ...
    system_gas =  mlt.System(mlt.Molecule('gly_gas.xyz', charge=1, mult=1),
                        box=None)
    ...
    system_sol = mlt.System(mol_CCl2,
                        box=mlt.box.Box([14.5, 14.5, 14.5]))
    ...
    system = mlt.System(mlt.Molecule('gly_gas.xyz', charge=1, mult=1),
    ...
    fix_reactant = [i for i in range(mlt.Molecule('gly_gas.xyz', charge=1, mult=1).n_atoms)]
    ace_ex.al_train(
        method_name="orca",
        temp=300,
        selection_method=selector,
