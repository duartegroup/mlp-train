...
class MACE(MLPotential):
    ...
    def _train_file(self) -> None:
        """
        This code is adjusted from run_train.py in mace package. For more details, see
        ...
        device = tools.init_device(device_str = Config.mace_params['device'])
        tools.set_default_dtype(args.default_dtype)

        try:
            Config.mace_params['config_type_weights'] = ast.literal_eval(config_type_weights)
            assert isinstance(Config.mace_params['config_type_weights'], dict)
        except Exception as e:  # pylint: disable=W0703
            logging.warning(
               f"Config type weights not specified correctly ({e}), using Default"
            )
            Config.mace_params['config_type_weights'] = {"Default": 1.0}

        # training and validate data preparation
        train_configs, valid_configs = self.get_dataset_from_xyz(train_path=args.train_file,
                                                ...
                                                config_type_weights=Config.mace_params['config_type_weights'],
                                                seed=args.seed,
        ...
        train_loader = torch_geometric.dataloader.DataLoader(
            dataset=[
                data.AtomicData.from_config(config, z_table=z_table, cutoff=Config.mace_params['r_max'])
        ...
        if Config.mace_params['loss'] == "weighted":
            ...
            loss_fn = modules.WeightedForcesLoss(forces_weight=5.0)
        else:
            loss_fn = modules.EnergyForcesLoss(
            energy_weight=args.energy_weight, forces_weight=5.0
            )
        ...
        model.to(Config.mace_params['device'])
        ...
        if Config.mace_params['restart_latest']:
            ...
            state=tools.CheckpointState(model, optimizer, lr_scheduler), device=Config.mace_params['device']
            )
        ...
        if Config.mace_params['ema']:
            ema = ExponentialMovingAverage(model.parameters(), decay=Config.mace_params['ema_decay'])
        ...
        swa=Config.mace_params['swa'],
        ema=Config.mace_params['ema'],
        ...
        Config.mace_params['error_table'],
        all_collections,
        z_table,
        Config.mace_params['r_max'],
        Config.mace_params['batch_size'],
        model,
        loss_fn,
        Config.mace_params['device'])
        # Save entire model
