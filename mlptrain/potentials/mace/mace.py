...
class MACE(MLPotential):
    ...
    def __init__(self, name: str, system: "mlptrain.System",) -> None:
        """
        ...
        try:
            ...
            raise ModuleNotFoundError(
                "MACE install not found, install it here: "
    ...
    def _save_model(self) -> None:
        ...
        logging.info(
            f"Saving the model {self.filename} to {self.args.checkpoints_dir} "
    ...
    def _set_train_objs(self) -> None:
        ...
        if self.args.compute_avg_num_neighbors:
            logging.info(
                f"Average number of neighbors: {self.avg_num_neighbors:.3f}"
            )

        logging.info(f"Model: {self.model}")
        logging.info(
            f"Number of parameters: {tools.count_parameters(self.model)}"
        )
    ...
    def scheduler(self) -> "torch.optim.lr_scheduler":
        ...
        if self._scheduler is None:
            ...
            if self.args.scheduler == "ExponentialLR":
                ...
                raise RuntimeError(
                    f"Unknown scheduler: {self.args.scheduler}"
                )
