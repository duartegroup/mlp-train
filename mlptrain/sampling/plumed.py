        ...
        for filename in cv_filenames:
            if not os.path.exists(filename):
                raise FileNotFoundError(
                    f"File {filename}, which is required for defining one "
                    "of the CVs was not found in the current directory"
                )
...
class _PlumedCV:
    ...
    def __init__(
        ...
        if filename is not None:
            ...
            raise TypeError(
                "Collective variable instantiation requires groups of atom "
                "indices (DOFs) or a file containing PLUMED-type input"
            )
    ...
    def _attach_files(self, filenames: List[str]) -> None:
        ...
        for filename in filenames:
            if not os.path.exists(filename):
                raise FileNotFoundError(
                    f"File {filename}, which is required for defining the CV "
                    f"{self.name} was not found in the current directory"
                )
    ...
    def _atom_group_to_dof(self, idx: int, atom_group: Sequence) -> None:
        ...
        if len(atom_list) > 4:
            raise NotImplementedError(
                "Instatiation using atom groups is only implemented for "
