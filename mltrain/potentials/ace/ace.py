...
class ACE(MLPotential):
    ...
    def _train(self) -> None:
        ...
        p = Popen([shutil.which('julia'), f'{self.name}.jl'],
                  shell=False,
                  stdout=PIPE,
                  stderr=PIPE)
    ...
    def ase_calculator(self) -> 'ase.calculators.calculator.Calculator':
        ...
        try:
            ...
            raise RuntimeError('Failed to import pyjulip required for '
    ...
    def _print_input(self,
        ...
        for species in self.system.unique_atomic_symbols:
            ...
            print(f'(3, z{species}) => {16 if species != "H" else 0},',
        ...
        for symbol in self.system.unique_atomic_symbols:
            print(f':{symbol} => {self.atomic_energies[symbol]},', file=inp_file)
        print(');', file=inp_file)

        # load the training data
        print(f'train_data = IPFitting.Data.read_xyz("{self.name}_data.xyz",\n'         
              '                                     energy_key="energy",\n'
              '                                     force_key="forces",\n'
              '                                     virial_key="dummy");\n',
              file=inp_file)
