...
\sffamily{\textbf{Machine learned potentials (MLPs) for reactions will enable reaction rates to be estimated quickly and accurately. Here, we show that recent advancements in MLP regression methods enable quantum dynamics and umbrella sampling to be performed routinely by leveraging automated fitting and active learning. Product distributions of ambimodal reactions are strongly dependent on the QM method and less so on the type of dynamics propagated. Reaction free energies from umbrella sampling for reactions containing 10s of atoms are now routinely available.}}\\%The abstrast goes here instead of the text "The abstract should be..."
